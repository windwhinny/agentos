...
import { BudgetExceededError, InterruptedError, KilledError, TermExit, TimeoutError } from '../errors';
...
export class Process extends EventEmitter {
  ...
  appendMessage(msg: ChatMessage): void {
    ...
    this.context.messages.push(Object.freeze(msg));
  }
   ...
   * （与 node 内核 src/core/process.ts 的 revive 保持一致）
   */
  revive(): void {
    if (!this.isExited) return;
    this.state = 'ready';
  ...
  private toExit(err: unknown): ExitResult {
    const base = { pid: this.pid, output: this.lastOutput, usage: { ...this.usage }, turns: this.turns };
    if (err instanceof TermExit) return { ...base, status: 'done' as const, reason: 'SIGTERM', exitCode: 0 };
    if (err instanceof BudgetExceededError) {
      const reason = err.kind === 'turns' ? 'MAX_TURNS' : err.kind === 'wall' ? 'TIMEOUT' : 'BUDGET_EXCEEDED';
    ...
    if (
      ...
      return { ...base, status: 'killed' as const, reason: 'SIGKILL', exitCode: 137, error: 'killed' };
    }
    const e = err as Error;
    return { ...base, status: 'failed' as const, reason: 'ERROR', exitCode: 1, error: e?.message ?? String(err) };
  }
  ...
  protected finish(result: ExitResult): void {
    ...
    this.stdout.push({ type: 'result', data: result.output, ts: Date.now() });
    this.stdin.close();
  ...
  private async callModel(): Promise<ChatResponse> {
    ...
    if (!this.provider.chatStream) {
      ...
      if (res.message.content || res.message.reasoning) {
        this.stdout.push({
          type: 'assistant',
          data: { text: res.message.content ?? '', ...(res.message.reasoning ? { thinking: res.message.reasoning } : {}) },
          ts: Date.now(),
        });
      }
      return res;
    ...
    try {
      ...
      if (this.flags.interrupt && !this.abort.signal.aborted) throw new InterruptedError(text, thinking);
      return res;
    } catch (e) {
  ...
  private async onInterrupted(e: InterruptedError): Promise<void> {
    ...
    this.appendMessage({ role: 'user', content: '（生成被用户中断）', meta: { from: 0, kind: 'interrupt' } });
    this.stdout.push({ type: 'stderr', data: '⏹ 生成被用户中断', ts: Date.now() });
  ...
  join(opts?: { timeoutMs?: number }): Promise<ExitResult> {
    ...
    return Promise.race([
      ...
      new Promise<ExitResult>((_, reject) =>
        setTimeout(() => reject(new TimeoutError(`join timeout after ${opts.timeoutMs}ms`)), opts.timeoutMs),
      ),
    ]);
  }
  ...
  async exec(opts: SpawnOptions): Promise<ExitResult> {
    ...
    this.spawnOptions = opts;
    this.modelParams = { ...this.runtime.defaults.model, ...this.parent?.modelParams, ...opts.model };
  ...
  private repairDanglingToolCalls(): void {
    const answered = new Set(
      this.context.messages.filter((m) => m.role === 'tool' && m.tool_call_id).map((m) => m.tool_call_id),
    );
