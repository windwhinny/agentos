...
export function makeBuiltinTools(): Map<string, Tool> {
  const tools: Tool[] = [
    {
      name: 'spawn_process',
      description: 'Spawn a child process (subagent) to run a task asynchronously. Returns { pid }.',
      parameters: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      execute: async (args, ctx) => {
        ...
        try {
          return await target.join({ timeoutMs: args.timeoutMs !== undefined ? Number(args.timeoutMs) : undefined });
        } finally {
