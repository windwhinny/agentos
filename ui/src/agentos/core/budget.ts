...
export class Budget {
  ...
  consumeTokens(n: number): void {
    ...
    if (this.quota.tokens !== undefined && this._usedTokens > this.quota.tokens) {
      throw new BudgetExceededError('tokens', `token budget exceeded (${this._usedTokens}/${this.quota.tokens})`);
    }
  ...
  consumeTurn(): void {
    ...
    if (this.quota.turns !== undefined && this._usedTurns > this.quota.turns) {
      throw new BudgetExceededError('turns', `turn budget exceeded (${this._usedTurns}/${this.quota.turns})`);
    }
